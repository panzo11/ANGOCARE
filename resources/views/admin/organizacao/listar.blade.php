                                ...
                                <tbody>
                                   @foreach ($organizacoes as $organizacao)
                                       <tr>
                                        ...
                                        <td class="text-center"><img class="bg-soft-primary rounded img-fluid avatar-40 me-3" src="{{ asset( $organizacao->logotipo) }}" alt="profile" loading="lazy"></td>
                                        <td>{{ $organizacao->nome }}</td>
                                        <td>{{ $organizacao->proprietario }}</td>
                                        <td>{{ $organizacao->iban }}</td>
                                        <td><a href="{{ route('organizacoes.documentos',$organizacao->id) }}">
                                        ...
                                        <td>
                                            ...
                                            <a class="btn btn-sm btn-icon btn-warning rounded" data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="Editar" href="{{route('organizacoes.edit', $organizacao->id)}}" >
                                             ...
                                             <a class="btn btn-sm btn-icon btn-danger rounded" data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="Eliminar" href="{{route('organizacoes.destroy', $organizacao->id)}}">
                                                <span class="btn-inner">
                                       ...
                                       <path fill-rule="evenodd" clip-rule="evenodd" d="M11.7281 21.9137C11.8388 21.9715 11.9627 22.0009 12.0865 22C12.2103 21.999 12.3331 21.9686 12.4449 21.9097L16.0128 20.0025C17.0245 19.4631 17.8168 18.8601 18.435 18.1579C19.779 16.6282 20.5129 14.6758 20.4998 12.6626L20.4575 6.02198C20.4535 5.25711 19.9511 4.57461 19.2082 4.32652L12.5707 2.09956C12.1711 1.96424 11.7331 1.96718 11.3405 2.10643L4.72824 4.41281C3.9893 4.67071 3.496 5.35811 3.50002 6.12397L3.54231 12.7597C3.5554 14.7758 4.31448 16.7194 5.68062 18.2335C6.3048 18.9258 7.10415 19.52 8.12699 20.0505L11.7281 21.9137ZM10.7836 14.1089C10.9326 14.2521 11.1259 14.3227 11.3192 14.3207C11.5125 14.3198 11.7047 14.2472 11.8517 14.1021L15.7508 10.2581C16.0438 9.96882 16.0408 9.50401 15.7448 9.21866C15.4478 8.9333 14.9696 8.93526 14.6766 9.22454L11.3081 12.5449L9.92885 11.2191C9.63186 10.9337 9.15467 10.9367 8.8607 11.226C8.56774 11.5152 8.57076 11.98 8.86775 12.2654L10.7836 14.1089Z" fill="currentColor"></path>
                                             ...
                                             <a class="btn btn-sm btn-icon btn-danger rounded" data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="Aprovar" href="{{route('organizacao.desativar', $organizacao->id)}}">
                                               <span class="btn-inner">
                                      ...
                                      <a class="btn btn-sm btn-icon btn-danger rounded" data-bs-toggle="tooltip" data-placement="top" title="" data-bs-original-title="Desativar" href="{{route('organizacao.desativar', $organizacao->id)}}">
                                         <span class="btn-inner">
...
@endif
@if (session('doc.error'))
<script>
    Swal.fire(
        'Erro ao fazer upload dos documentos!',
        'certifique-se de que adicionou um documento valido',
        'error'
    )
</script>
@endif
...
<script>
    Swal.fire(
        ...
        'certifique-se de que adicionou uma Imagem valida',
        'error'
    )
</script>
@endif
{{-- Fim organização alert --}}

{{-- Organização delete Alert --}}
@if (session('delete'))
    <script>
        Swal.fire(
...
@endif
{{-- Fim Organização delete Alert --}}
{{-- Organização estado Alert --}}
@if (session('on'))
    <script>
        Swal.fire(
            'Organização Aprovada com sucesso!',
            '',
            'success'
        )
    </script>
...
@if (session('off'))
    <script>
        Swal.fire(
            'Organização Rejeitada com sucesso!',
            '',
            'success'
        )
    </script>
...
<script>
    Swal.fire(
        'Erro ao Rejeitar Organização!',
        '',
        'error'
    )
</script>
@endif
{{-- Fim Organização estado Alert --}}
@endsection
