...
class CausasController extends Controller
{
    ...
    public function index(){
       ...
       foreach( $data['doacoes'] as $fundo){
        ...
        return view('site.causes.index',$data);
    }
    public function show($id){
        $doacao = DB::table('financiamentos')
        ->leftJoin('doacao_financimentos', 'doacao_financimentos.financiamentos_id', '=', 'financiamentos.id')
        // ->where('financiamentos_id', $id)  // Descomente e ajuste conforme necessário
        ->select(
            'financiamentos.id',
        ...
        ->where('financiamentos.id',$id)
        ->first();
        
        return  view('site.causes.single', compact('doacao'));
    }
    ...
    public function show2($id){
        // dd('ola');
            ...
            $doacao = DB::table('produtos')->join('users','produtos.users_id','users.id')
            ->select('users.name as utilizador','produtos.*')->first();

            return  view('site.causes.single2', compact('doacao'));
    }
}
